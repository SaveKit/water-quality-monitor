...
import { Calendar, Search, Download, Database, Check, Layers, AlertCircle } from "lucide-react";
...
export default function Analytics() {
  ...
  const fetchHistoricalData = async () => {
    ...
    try {
      // Parse dates to standard ISO strings format (ending with Z)
      const startIso = new Date(startDate).toISOString();
      const endIso = new Date(endDate).toISOString();

      // Query database for all combinations of active parameters and nodes
      const promises = [];
      const keys = [];
  ...
  // Fetch initial analytics on component mount
  useEffect(() => {
    fetchHistoricalData();
  }, []);
  ...
  const handleExportCSV = async (node, param) => {
    try {
      ...
      const response = await client.get("/api/data/export/csv", {
        ...
        responseType: "blob", // handle binary response file
      });

      // Create browser download link trigger
  ...
  // Compile raw data points list sorted DESC for displaying in a table below the chart
  const getCombinedRawData = () => {
    ...
    return combined.sort((a, b) => b.timestamp - a.timestamp).slice(0, 100); // show top 100
  };
    ...
    <div className="min-h-screen bg-[#070913] flex">
      ...
      <main className="flex-1 pl-64 p-8 min-h-screen">
        ...
        <header className="mb-8">
          <h1 className="text-2xl font-black text-white">วิเคราะห์ข้อมูลเชิงลึก (Analytics)</h1>
        ...
        <section className="glass-panel border border-slate-800 p-6 rounded-3xl mb-8 space-y-6">
          ...
          <div className="flex justify-end pt-2 border-t border-slate-800/40">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-bold rounded-xl px-6 py-3 text-xs transition-all duration-300 disabled:opacity-50"
            >
                      ...
                      <div className="flex gap-2">
                        {activeNodes.map((node) => (
                          <button
                            ...
                            className="flex items-center gap-1.5 bg-slate-800/50 hover:bg-slate-800 text-slate-300 border border-slate-700/50 rounded-xl px-3 py-1.5 text-[10px] font-bold transition-all duration-300"
                          >
